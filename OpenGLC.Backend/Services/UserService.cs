...
namespace OpenGLC.Backend.Services
{
    public class UserService : IUserService
    {
        ...
        public async Task<TokenResultModel> Login(string userName, string password, bool? tokenForDeleteAction = false)
        {
            ...
            if (string.IsNullOrWhiteSpace(user.HashedPassword) || string.IsNullOrEmpty(user.Salt))
            {
        ...
        public async Task<TokenResultModel> ExternalProviderLogin(string userName)
        {
            User? user = await GetUserByUserName(userName);
        ...
        public async Task<User?> GetUserByUserName(string userNameOrEmail)
        {
            var user = await _userRepository.FindByExpresion(u => u.UserName == userNameOrEmail || u.Email == userNameOrEmail).FirstOrDefaultAsync();
            return user;
        }
        ...
        public async Task<EncryptorResultModel> RegisterUser(NewRegisterModel newRegister)
        {
            ...
            var encryptedPassword = new EncryptorResultModel() { PasswordHash = "", SaltValue = "" };

            if (!string.IsNullOrWhiteSpace(newRegister.Password))
                encryptedPassword = await _encryptor.PasswordEncrypt(newRegister.Password);
