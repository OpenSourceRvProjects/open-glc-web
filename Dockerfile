...
RUN dotnet publish "OpenGLC.MVC/OpenGLC.MVC.csproj" -c Release -o /app/publish /p:EnvironmentName=QA
...
EXPOSE 80
EXPOSE 443
# Inject the environment variable into the web.config during container build
COPY --from=publish /app/publish .

# Set the entry point to the .NET application
ENTRYPOINT ["dotnet", "OpenGLC.MVC.dll"]